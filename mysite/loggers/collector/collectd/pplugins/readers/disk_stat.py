...
class DiskStat:
    ...
    def dispatch_collectd(self, name, read_count, write_count, read_bytes, write_bytes):
        rate = collectd.Values()
        rate.plugin = 'disk_stat'
        rate.plugin_instance = name
        rate.interval = int(self.interval)
        rate.type = 'disk_stat_rate'
        rate.values = [read_count, write_count, read_bytes, write_bytes]
        rate.dispatch()
    
        counter = collectd.Values()
        counter.plugin = 'disk_stat'
        counter.plugin_instance = name
        counter.interval = int(self.interval)
        counter.type = 'disk_stat_counter'
        counter.values = [read_bytes, write_bytes]
        counter.dispatch()
    ...
    def read(self):
        ...
        if err:
            collectd.debug('disk_stat plugin: error in lsblk command')
            return

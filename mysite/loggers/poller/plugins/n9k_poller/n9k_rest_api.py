...
class N9k_rest_api(object):
    ...
    def session_post(self, session, url):
        ...
        try:
            ...
            if not resp.ok:
                return int(resp.status_code), resp.reason, resp.text
            result = resp.json()
            if result["ins_api"]["outputs"]["output"]["code"] != "200":
                return result["ins_api"]["outputs"]["output"]["code"], \
                       result["ins_api"]["outputs"]["output"]["msg"], \
                       ""
            else:
                return result["ins_api"]["outputs"]["output"]["code"], \
                       result["ins_api"]["outputs"]["output"]["msg"], \
                       result["ins_api"]["outputs"]["output"]["body"]
        except requests.exceptions.RequestException as ex:
            return 404, "Exception", str(ex)
    ...
    def show_interface(self, session, url, interface):
        ...
        if int(status_code) != 200:
            return status_code, message, body
        return status_code, message, body["TABLE_interface"]["ROW_interface"]
    ...
    def show_lldp_neighbors(self, session, url, interface):
        ...
        if int(status_code) != 200:
            return status_code, message, body
        return status_code, message, body["TABLE_nbor_detail"]["ROW_nbor_detail"]
    ...
    def clear_interface_counters(self, session, url, interface=None):
        ...
        if int(status_code) != 200:
            return status_code, message, body
        return status_code, message, "clear counters successful"
    ...
    def clear_buffer_counters(self, session, url):
        ...
        if int(status_code) != 200:
            return status_code, message, body
        return status_code, message, "clear counters buffer successful"
    ...
    def get_buffer_details(self, session, url, interface):
        ...
        if int(status_code) != 200:
            return status_code, message, body
        return status_code, message, body["TABLE_module"]["ROW_module"]["TABLE_instance"]["ROW_instance"]
...
def main():
    ...
    print api.show_interface(sess, url, interface="ethernet1/10")
    
    print api.show_interface_switchport(sess, url, interface="eth1/5")
    print api.show_interface_switchport(sess, url, interface="eth1/10")
    
    print api.show_lldp_neighbors(sess, url, interface="eth1/3")
    print api.show_lldp_neighbors(sess, url, interface="eth1/4")
    
    print api.show_cdp_neighbors(sess, url, interface="eth1/3")
    print api.show_cdp_neighbors(sess, url, interface="eth1/4")

...
Parameter:
            ...
            'application_id : <job-id>, # Fetch job based on tag provided while running application
    ...
    def __application_details(self):
        ...
        if app_list:
            for app in app_list.get('app'):
                ...
                if (self.application_names is not None) and (app.get('name') in self.application_names):
                    continue
        ...
        self.__update_result(app_results)
...
if __name__ == '__main__':
    import time
    config = dict(
        {'resource_manager_address': '192.168.100.205', 'port': 8088, 'application_names': ['testdfsio_template1']})

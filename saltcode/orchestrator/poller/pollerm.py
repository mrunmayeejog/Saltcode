...
class PollerManager:
    ...
    def check_status(self, plugin_id=""):
        ...
        partitions = self.pcon_comm_consumer.poll(10000)
        if len(partitions) > 0:
            for p in partitions:
                if not plugin_id:
                    return json.loads((partitions[p][-1].value))[CHECK_POINT][STATE]
                else:
                    return json.loads((partitions[p][-1].value))[CHECK_POINT][STATE][plugin_id]
    ...
    def build_msg(self, msg, inst_type=PLUGIN, op=START):
        ...
        if op == TEARDOWN:
            msg.update({PLUGIN_ID: TEARDOWN})
        if op == START or op == UPDATE or op == STOP or op == RESUME:
...
#msg = {"targets":[{"msg_type":"cntrl", "plugin_id":"kafka_writer", "target":"kpark", "plugin_type":"writer", "cmd":"start", "mode":"frill", "meta":{"kafka_client":"192.168.101.12:9092", "topic":"ptop1"}}], "plugins":[{"msg_type":"cntrl", "plugin_id":"reader_plugin", "target":"poll", "plugin_type":"reader", "cmd":"start", "mode":"frill", "target_list":["kafka_writer"], "meta":{"interval":10, "tag":"red", "run_count":10}}]}
#msg = {"targets":[{"msg_type":"cntrl", "name":"kafka_writer", "target":"kpark",  "cmd":"start", "mode":"frill", "meta":{"kafka_client":"192.168.101.12:9092", "topic":"ptop1"}}], "plugins":[{"msg_type":"cntrl",  "name":"hadoop1" ,"target":"poll", "cmd":"start", "mode":"frill", "targets":["kpark"], "interval":10, "tag":["red"],"meta":{}}]}

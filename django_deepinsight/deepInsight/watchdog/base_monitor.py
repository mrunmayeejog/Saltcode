...
class BaseMonitor:
    ...
    def read_template_data(self):
        ...
        try:
            with open(filepath, 'r') as f:
                ...
                self.nodelist = template.get('node_list', [])
                self.sub_template = template.get('sub_template')[0].get(self.name)
        except:
    ...
    def change_service_status(self, operation):
        try:
            ...
            self.logger.error('Exception Caused !!')
            return
...
class SaltManager:
    ...
    def change_service_status(self, nodes, services, status):
        ...
        status = self.local.cmd(tgt=nodes, expr_form='list',
                                fun=SERVICE_STATUS[status], arg=services)

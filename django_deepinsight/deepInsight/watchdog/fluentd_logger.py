...
class FluentdPluginManager(BaseMonitor):
    ...
    def __init__(self, tid):
        super(FluentdPluginManager, self).__init__(FluentdPluginManager._NAME_, tid)
        ...
        config_file = settings.CONF_DIR + os.path.sep + 'plugin_config.json'
    ...
    def deploy(self, oper):
        ...
        if oper == START:
            if self.sub_template.get('enable', False):
                ...
                self.logger.debug('self.plugin_post_data' + json.dumps(self.plugin_post_data))
            ...
            if self.sub_template.get('enable', False):
                ...
                self.logger.debug('self.plugin_post_data' + json.dumps(config_post_data))
                config_status = salt_obj.push_config(config_post_data, self.nodelist)
                self.logger.debug('Config Push Status :' + json.dumps(config_status))
